	...
	if (ev->window == root) {
		...
		if (updategeom() || dirty) {
			...
			for (m = mons; m; m = m->next) {
				...
				/* Resize/move bar windows to match floatbar or normal bar sizing */
				if (floatbar) {
					int bw = m->ww - 2 * barpadh - 2 * barborder;
					int bx = m->wx + barpadh;
					int by = m->by;
					XMoveResizeWindow(dpy, m->barwin, bx, by, bw, bh + barheight);
				} else {
...
updatebars(void)
{
	...
	for (m = mons; m; m = m->next) {
	    ...
	    if (floatbar) {
	        ...
	        int bx = m->wx + barpadh;
	        int by = m->by;
	        m->barwin = XCreateWindow(dpy, root, bx, by, bw, bh + barheight, 0, DefaultDepth(dpy, screen),
...
updatebarpos(Monitor *m)
{
	if (floatbar) {
	    ...
	    m->wy = m->my + (barheight + bh + barpadv + barborder); 	/* Start window area below the bar */
	    m->wh = m->mh - (barheight + bh + barpadv + barborder); 	/* Reduce window height to account for bar */
...
main(int argc, char *argv[])
{
	...
	scan();
	run();
    if (restarting) execvp(argv[0], argv);
	cleanup();
	XCloseDisplay(dpy);
	return EXIT_SUCCESS;
}
